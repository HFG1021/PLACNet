...
class DeformAtten1D(nn.Module):
    ...
    '''
    def __init__(self, seq_len, d_model, n_heads, dropout, kernel, n_groups=4, no_off=False, rpb=True) -> None:
        super().__init__()
...
class CrossDeformAttn(nn.Module):
    def __init__(self, seq_len, d_model, n_heads, dropout, droprate,kernel,
        ...
        # Deform attention
        self.deform_attn = DeformAtten1D(self.subseq_len, d_model, n_heads, dropout, kernel, no_off=no_off)
...
if __name__ == '__main__':
    # (B,L,D)
    x1 = torch.randn(1,81,544).to(device)

    Model = CrossDeformAttn(seq_len=81, d_model=544, n_heads=8, dropout=0.,kernel=7, droprate=0.,n_days=1).to(device)
